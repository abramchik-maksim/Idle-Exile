...
{
    public sealed class FileTreeTalentsRepository : ITreeTalentsRepository
    {
        private readonly ISaveSlotManager _slotManager;

        public FileTreeTalentsRepository(ISaveSlotManager slotManager)
        {
        ...
        public void Save(TreeTalentsState state)
        {
            ...
            File.WriteAllText(
                FileSavePaths.TreeTalentsPath(_slotManager.ActiveSlotIndex),
                JsonUtility.ToJson(data, true));
        }

        public TreeTalentsState Load()
        {
            var path = FileSavePaths.TreeTalentsPath(_slotManager.ActiveSlotIndex);
            if (!File.Exists(path))
        ...
        private static List<BranchInstance> DeserializeBranches(List<BranchData> data)
        {
            ...
            foreach (var branchData in data)
            {
                ...
                if (branchData.isPlaced)
                    branch.PlaceAt(new GridCoord(branchData.anchorX, branchData.anchorY), branchData.rotationQuarterTurns);
